...
function Write-HookLog([string]$message) {
  ...
  $logPath = Join-Path $logDir "failure-learning-capture.log"
...
if ($text -match "Failure Learning Capture triggered" -or
    $text -match "Before sending the final answer, do a short learning-capture pass") {
  Write-StopResult ""
  exit 0
}
...
$stateDir = Join-Path $env:USERPROFILE ".codex\hooks\state\failure-learning-capture"
...
$captureDir = Join-Path $env:USERPROFILE ".codex\hooks\learning-captures"
...
$message = @"
Failure Learning Capture triggered.
...
Before sending the final answer, do a short learning-capture pass:
